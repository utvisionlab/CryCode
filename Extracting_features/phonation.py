...
import math
import pysptk
# import Extracting_features.phonation_functions as pf
...
# import Extracting_features.utils as util
...
# import Extracting_features.praat_functions as praat_func
from praat_functions import praat_vuv,decodeF0
...
def phonationVowels(audio, flag_plots, size_frame=0.02,size_step=0.01,minf0=60,maxf0=1000, voice_bias=-0.2,energy_thr_percent=0.025, pitch_method='rapt'):
    ...
    if pitch_method == 'praat':
        ...
        audio_new='/'.join(name_audio)
        temp_uuid='phon'+name_audio[-2]+name_audio[-1][0:-4] # Modified by Aida
        if not os.path.exists('/code/tempfiles/'):
            os.makedirs('/code/tempfiles/')
        temp_filename_vuv='/code/tempfiles/tempVUV'+temp_uuid+'.txt'
        temp_filename_f0='/code/tempfiles/tempF0'+temp_uuid+'.txt'
        praat_vuv(audio_new, temp_filename_f0, temp_filename_vuv, time_stepF0=size_step, minf0=minf0, maxf0=maxf0)
        F0,_=decodeF0(temp_filename_f0,len(data_audio)/float(fs),size_step)
