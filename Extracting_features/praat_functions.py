...
from os import getcwd, path # By Aida
def praat_vuv(audio_filaname, resultsp, resultst, time_stepF0=0, minf0=75, maxf0=600, maxVUVPeriod=0.02, averageVUVPeriod=0.01):
	...
	command= path.join(getcwd(),'Extracting_features','Praat.exe ')+ path.join(getcwd(),'Extracting_features','vuv_praat.praat ')
...
def praat_formants(audio_filename, results_filename,sizeframe,step, n_formants=5, max_formant=5500):
	...
	Returns:
		Nothing
	"""
	command= path.join(getcwd(),'Extracting_features','Praat.exe ')+ path.join(getcwd(),'Extracting_features','FormantsPraat.praat ')
